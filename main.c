...
int main(void)
{
	...
	Uart1_Init();
	//SSD1306_init();   //|
	SSD1351_init();     //SSD1351和SSD1306只可使用其一，当你的显示有问题时请检查是不是SSD1351用了SSD1306的代码了等之类的问题
	Lcd12864_Init();
	KEY_BOARD_init();
	INA_init();
...
//	TA0_PWM_init();     //TA0.1即P1.2的SPWM(你可以理解为占空比呈正弦变化的PWM)输出测试初始化，初始化为即开始输出，注意拔掉对应矩阵键盘跳线帽在进行测试以免影响键盘使用
	...
	//SSD1306_ShowString(0, 0, "Hello MSP430F5529!", 8);
...
//	DAC8571_OUT(2.5);
    ...
    while(1)
    {
        ...
        sprintf(display_str, "U: %.2f V  ", Vlotage);
        SSD1351_Show_String(0, 12, display_str, WHITE, GREEN, 12, 0);
        sprintf(display_str, "I: %.2f A  ", Current);
        SSD1351_Show_String(0, 24, display_str, WHITE, GREEN, 12, 0);
        sprintf(display_str, "P: %.2f W  ", Power);
        SSD1351_Show_String(0, 36, display_str, WHITE, GREEN, 12, 0);
